...
function setValueAt(index, value) {
  const row = Math.floor(index / LEVEL_SCHEMA.columns);
  const column = index % LEVEL_SCHEMA.columns;
  layout[row] = layout[row].slice(0, column) + value +
    layout[row].slice(column + 1);
}
...
function applyBrickStyle(cell, code) {
  ...
  const action = selectedTool === "cycle"
...
function renderGrid() {
  ...
  for (
    let index = 0;
    index < LEVEL_SCHEMA.rows * LEVEL_SCHEMA.columns;
    index += 1
  ) {
    ...
    cell.setAttribute(
      "aria-rowindex",
      String(Math.floor(index / LEVEL_SCHEMA.columns) + 1),
    );
    cell.setAttribute(
      "aria-colindex",
      String((index % LEVEL_SCHEMA.columns) + 1),
    );
...
function updateValidation(showErrors = false) {
  ...
  elements.levelNameError.textContent = showErrors
...
function focusCell(cell) {
  for (
    const candidate of elements.grid.querySelectorAll(
...
function continuePaint(event, knownCell = null) {
  ...
  if (!cell) return;
  const index = Number(cell.dataset.index);
  const indexes = previousPaintIndex === null
    ? [index]
  ...
  for (const paintIndex of indexes) {
    ...
    const paintTarget = elements.grid.querySelector(
      `[data-index="${paintIndex}"]`,
    );
...
function setStatus(state, code, message, link = null) {
  ...
  if (link) {
    const deepLink = communityLevelUrl(link.id);
    const shareLink = link.shareUrl;
    const sharePanel = document.createElement("div");
    sharePanel.className = "share-panel";
    const urlField = document.createElement("input");
    urlField.className = "share-url";
    urlField.type = "url";
    urlField.readOnly = true;
    urlField.value = shareLink;
    urlField.setAttribute("aria-label", `Share link for ${link.levelName}`);
    const actions = document.createElement("div");
    actions.className = "share-actions";
    const anchor = document.createElement("a");
    anchor.href = deepLink;
    anchor.textContent = "PLAY THIS LEVEL";
    const shareButton = document.createElement("button");
    shareButton.className = "share-link-action";
    shareButton.type = "button";
    shareButton.dataset.shareUrl = shareLink;
    shareButton.dataset.levelName = link.levelName;
    shareButton.textContent = typeof navigator.share === "function"
      ? "SHARE LEVEL"
      : "COPY LEVEL LINK";
    actions.append(anchor, shareButton);
    sharePanel.append(urlField, actions);
    elements.status.append(sharePanel);
  }
}

async function resolveCommunityShareUrl(levelId) {
  try {
    const response = await fetch(
      `/api/resolve?id=${encodeURIComponent(levelId)}`,
    );
    if (!response.ok) {
      throw new Error(`Community catalog returned ${response.status}.`);
    }
    const body = await response.json();
    if (typeof body?.share_url !== "string") {
      throw new Error("Community share response was invalid.");
    }
    return body.share_url;
  } catch (error) {
    console.warn(
      "Could not resolve a friendly share URL; using the stable id route.",
      error,
    );
    return communityLevelFallbackUrl(levelId);
  }
}
...
async function shareLevelLink(button) {
  ...
  if (typeof navigator.share === "function") {
    try {
      ...
      console.warn(
        "Native sharing failed; falling back to the clipboard.",
        error,
      );
    }
  }
  ...
  if (typeof navigator.clipboard?.writeText !== "function") {
    button.textContent = "SELECT LINK ABOVE";
    return;
  }
  try {
    ...
    console.error("Could not copy the community level link.", error);
...
function friendlyError(status, body) {
  if (status === 429) {
    ...
    return retry > 0
      ? `Submission limit reached. Try again in ${
        Math.ceil(retry / 60)
      } minutes.`
  ...
  if (status === 422 || status === 400) {
    return body?.message ||
      "The server rejected this level. Check the labels and grid.";
  }
  if (status >= 500) {
    return "The Community Lab is unavailable. Your grid is still here.";
  }
  return body?.message ||
...
async function submitLevel(event) {
  ...
  if (!result.valid) {
    setStatus(
      "error",
      "CHECK LEVEL",
      result.errors.layout || "Fix the labeled fields.",
    );
    (result.errors.level_name
      ? elements.levelName
      : result.errors.creator_display_name
      ? elements.creatorName
      : elements.grid.querySelector(".grid-cell"))?.focus();
    return;
  }
  ...
  setStatus(
    "loading",
    "TRANSMITTING",
    "Sending the immutable cartridge to the Lab.",
  );
  ...
  try {
    ...
    const shareUrl = await resolveCommunityShareUrl(body.id);
    setStatus(
      "success",
      ...
      {
        id: body.id,
        levelName: result.normalized.level_name,
        shareUrl,
      },
    );
...
function armOrClearGrid() {
  ...
  setStatus(
    "ready",
    "GRID CLEARED",
    "The cartridge is blank and ready for a new layout.",
  );
}
...
elements.status.addEventListener("click", (event) => {
  const shareButton = event.target.closest(".share-link-action");
  if (shareButton) void shareLevelLink(shareButton);
});
elements.status.addEventListener("focusin", (event) => {
  if (event.target.matches(".share-url")) event.target.select();
});
