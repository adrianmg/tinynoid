create index if not exists community_levels_share_slug_idx
on public.community_levels (
  (private.community_level_slug(level_name)),
  created_at,
  id
);

-- Any future change to private.community_level_slug must rebuild this index in
-- the same migration so expression-index values cannot become stale.
...
begin
  ...
  if p_id is not null then
    ...
    suffix_id := substring(p_slug from '(cl_[0-9a-f]{24})$');
    if suffix_id is not null and right(p_slug, 28) = '-' || suffix_id then
      requested_base_slug := left(p_slug, char_length(p_slug) - 28);
      select level.id
      into target_id
      from public.community_levels as level
      where level.id = suffix_id
        and private.community_level_slug(level.level_name)
          = requested_base_slug;
    else
      select level.id
      into target_id
      from public.community_levels as level
      where private.community_level_slug(level.level_name) = p_slug
