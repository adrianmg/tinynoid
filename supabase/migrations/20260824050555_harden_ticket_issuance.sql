create table private.score_ticket_limits (
  key_hash text primary key,
  window_started_at timestamptz not null,
  attempt_count integer not null
);

alter table private.score_ticket_limits enable row level security;

drop function public.issue_score_run(uuid, text);
...
begin
  ...
  delete from private.score_run_tickets
  where expires_at <= request_time
    or (
      consumed_at is not null
      and consumed_at <= request_time - interval '1 hour'
    );
...
revoke execute on function public.issue_score_run(uuid, text, text)
  from public, anon, authenticated;
grant execute on function public.issue_score_run(uuid, text, text)
  to service_role;
