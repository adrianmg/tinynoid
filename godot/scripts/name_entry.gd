...
const ACTION_KEYS := ["DELETE", "GUEST", "DONE"]
...
func _draw() -> void:
	...
	PixelFont.draw_centered(
		self,
		"SAVE SCORE AS" if _for_score_submission else "EDIT PLAYER",
		40,
		CYAN,
		2
	)
	...
	if not _player_name.is_empty():
		PixelAvatar.draw(self, _player_name, Vector2(44, 70))
	PixelFont.draw_centered(
		self,
		_player_name if not _player_name.is_empty() else "ENTER NAME",
		72,
	...
	PixelFont.draw_centered(self, "TYPE OR USE ARROWS / FIRE", 208, CYAN)
	PixelFont.draw_centered(self, "ESC DELETE / GUEST", 220, WHITE)
